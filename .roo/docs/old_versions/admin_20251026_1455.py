...
from .utils_admin_media_files import (
...
from routes.utils_admin_tvdb import tvdb_search_util
...
def media():
    ...
    # Added POST method support for file uploads
    if request.method == 'POST':
        print("DEBUG: Entered admin_media POST block.")
        # Import process_media_upload for handling POST requests
        from routes.utils_admin_media_files import upload_media_post_util
        result = upload_media_post_util()
        if result.get('success'):
            return redirect(url_for('admin.media'))
        else:
            # Handle error case - flash message and continue with GET display
            if 'message' in result:
                flash(result['message'], result.get('message_type', 'error'))
    
    # Handle GET request or fallback after POST
    return handle_util_result(media_management_util())

@admin_bp.route('/media/edit/<int:media_id>', methods=['GET', 'POST'])
@login_required
@admin_required
def media_edit(media_id):
    """Edit media items."""
    # Modified by Claude Sonnet 4.5 | 2025-10-22
    # Fixed route URL from /media_edit to /media/edit for consistency with other media routes
...
# Legacy redirect for old /admin/upload URLs
@admin_bp.route('/upload', methods=['GET', 'POST'])
@login_required
@admin_required
def upload_legacy_redirect():
    """Legacy redirect for old /admin/upload URLs to /admin/media/upload."""
    # Created by Claude Sonnet 4 | 2025-10-24
    # Preserve query parameters from the original request
    return redirect(url_for('admin.upload_media', **request.args), code=301)
...
@admin_bp.route('/tvdb_search', methods=['GET', 'POST'])
@login_required
@admin_required
def tvdb_search():
    ...
    if request.method == 'POST':
        ...
        if series_name:
            results_data = tvdb_search_util(series_name)
            if results_data.get('success'):
                search_results = results_data.get('data')
            else:
                flash(results_data.get('error', 'An unknown error occurred during TVDB search.'), 'danger')
    return render_template('admin/tvdb_search.html', search_results=search_results)
