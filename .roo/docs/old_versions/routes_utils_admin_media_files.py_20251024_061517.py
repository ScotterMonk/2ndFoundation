...
def upload_media_get_util():
    ...
    try:
        ...
        # 1. Fetch potential owner and media type from query params
        owner_user_id = request.args.get('owner_user_id', type=int)
        media_type_id = request.args.get('media_type_id', type=int)

        owner_user = db.session.get(User, owner_user_id) if owner_user_id else None
        media_type = db.session.get(MediaType, media_type_id) if media_type_id else None

        # 2. Load all media types for the dropdown
        media_types = MediaType.query.order_by(MediaType.name).all()

        # 3. Return successfully with data for the template
        return {
            'success': True,
        ...
        current_app.logger.error(f"Error in upload_media_get_util: {str(e)}", exc_info=True)
        return {
            'success': False,
            'error': "An error occurred while loading the upload page.",
            'status_code': 500
        }
...
def upload_media_post_util():
    ...
    try:
        ...
        for file in valid_files:
            # Debug: Log handler details before calling process_file
            current_app.logger.debug(f"DEBUG: handler type: {type(handler)}, handler class: {handler.__class__}")
            current_app.logger.debug(f"DEBUG: handler.process_file method: {type(handler.process_file)}")
            
            result = handler.process_file(file, owner.id, media_type.code)
            
            # Debug: Log result type to validate assumption
            current_app.logger.debug(f"DEBUG: result type: {type(result)}, result keys: {list(result.keys()) if hasattr(result, 'keys') else 'N/A'}")
            current_app.logger.debug(f"DEBUG: result content: {result}")
...
def select_owner_search_util():
    """
    Handle AJAX requests for searching users, using core helpers for
    querying, pagination, and data preparation.
    """
    try:
        from utils.user_query_core import build_user_search_query, prepare_user_list_data
        from utils.media_query_core import paginate_media_query
        ...
        # 1. Extract search parameters from request args
        show_all = request.args.get('show_all', '').lower() in ('true', '1', 'on', 'yes')
        resellers_only = request.args.get('resellers_only', '').lower() in ('true', '1', 'on', 'yes')
        search_term = request.args.get('q', '').strip()
        page = request.args.get('page', 1, type=int)

        # 2. Build the query using the core helper
        query = build_user_search_query(
            db.session, search_term,
            active_only=not show_all,
            resellers_only=resellers_only
        ...
        # 3. Paginate the results (using a smaller page size for AJAX)
        page_result = paginate_media_query(query, page, per_page=10)

        # 4. Prepare data for the JSON response
        user_data = prepare_user_list_data(page_result['items'])

        # 5. Return JSON response for the AJAX call
        return {
            'success': True,
            'results': user_data,
            'pagination': {
        ...
        # Use current_app.logger since no module-level logger is defined
        current_app.logger.error(f"Error in select_owner_search_util: {str(e)}", exc_info=True)
        return {'success': False, 'error': 'Failed to perform user search.'}