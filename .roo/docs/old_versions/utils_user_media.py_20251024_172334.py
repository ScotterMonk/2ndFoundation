...
def upload_media_for_user(files, media_type_code: str = 'image') -> Dict[str, Any]:
    """
    Handle media uploads for regular users using the new MediaFileHandler adapter pattern.
    
    This function processes uploaded files from users through the new adapter,
    which includes feature flag routing and shadow mode capabilities.
    It maintains the same interface as the original upload functions while
    delegating to the appropriate handler based on configuration.
    ...
    Returns:
        dict: Result containing success status, uploaded files info, and error messages
    """
    try:
        ...
        if not current_user or not current_user.is_authenticated:
            return {
                'success': False,
                'message': 'Authentication required for media uploads',
                'uploaded_files': [],
                'error_messages': ['Authentication required']
            }
        
        # Use the new adapter function for processing
        ...
        try:
            UserAction.log_action(
                user_id=current_user.id,
                action_type='user_media_upload',
                action_category='media',
                details={
                    'media_type_code': media_type_code,
                    'files_uploaded': len(successful_uploads),
                    'files_failed': len(error_messages),
                    'filenames': [upload.get('filename_original', 'unknown') for upload in successful_uploads]
                },
                ip_address=request.remote_addr,
                user_agent=request.headers.get('User-Agent', ''),
            ...
            current_app.logger.warning(f"Failed to log user upload action: {str(e)}")
        
        # Return standardized result
        return {
            'success': len(successful_uploads) > 0,
            'message': f"Successfully uploaded {len(successful_uploads)} file(s)" if successful_uploads else "No files were uploaded successfully",
        ...
        current_app.logger.error(f"Error in upload_media_for_user: {str(e)}")
        return {
            'success': False,
